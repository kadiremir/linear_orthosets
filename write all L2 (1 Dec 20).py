...
def is_los(arg1):
    ...
    for item1 in X:
        ...
        for item2 in [x for x in X if x != item1]:  # that means f is different than e
            ...
            if is_orthogonal(item1, [item2]):

                if any(orth_compl([item1, item2]) == orth_compl([item1, item3]) for item3 in complement(X, orth_compl([item1]))) is False:
